...
import { CoverLetterPdfDocument, ResumePdfDocument } from "@/features/resume/resume-pdf";
...
export function EditorPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        toast.error(getErrorMessage(error, "Failed to load editor data"));
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-semibold">Resume & Cover Letter Editor</h2>
                ...
                {resume.education.map((edu, idx) => (
                  <Card key={`edu-${idx}`} className="space-y-2">
                    ...
                    <Field
                      label="School"
                      value={edu.school}
                      onChange={(value) => updateEducation(idx, "school", value)}
                    />
                    <Field
                      label="Degree"
                      value={edu.degree}
